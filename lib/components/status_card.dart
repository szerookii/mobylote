...
class _StatusCardState extends State<StatusCard> {
  ...
  _handleSetAvailability(bool available) async {
    ...
    final result = await setAvailability(widget.id, available, DateFormat('yyyy-MM-dd').format(DateTime.now().add(const Duration(days: 7))));
    ...
    if(result) {
      showToast(context, 'Votre statut a bien été mis à jour', ToastType.success);
    } else {
      showToast(context, 'Erreur lors de la mise à jour de votre statut', ToastType.error);
    }
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: AnimatedSwitcher(
        ...
        child: profile == null ? _buildSkeleton() : Column(
          key: ValueKey<int>(1),
                ...
                children: [
                  Column(
                    ...
                    children: [
                      ...
                      profile!.meta.freelance.available ?
                      Text(
                        'pendant encore ${(DateTime.parse(profile!.meta.freelance.availabilityDate).difference(DateTime.now()).inDays)} jours',
                        style: const TextStyle(
                          color: Colors.grey,
                          fontSize: 14,
                        ),
                      ...
                      ElevatedButton(
                        style: ElevatedButton.styleFrom(
                          ...
                          side: const BorderSide(color: Color(0x52ABABAB), width: 0.5),
                          backgroundColor: Colors.transparent,
                          minimumSize: const Size(double.infinity, 50),
                        ),
                        onPressed: _isLoading ? null : () async => await _handleSetAvailability(false),
                        child: _isLoading
                      ...
                      ElevatedButton(
                        ...
                        onPressed: _isLoading ? null : () async => await _handleSetAvailability(true),
                        child: _isLoading
                            ? const CircularProgressIndicator(
                                valueColor: AlwaysStoppedAnimation<Color>(Colors.white),
                              )
                  ...
                  Positioned(
                    ...
                    child: IconButton(
                      ...
                      onPressed: () async => _isLoading ? null : await _handleSetAvailability(true)
                    ),
                  ),
                ],
              ),
